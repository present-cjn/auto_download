...
打开“下载设置”可以调整本机下载行为。常用项：

- 下载超时：网络慢或大文件夹多时可调到 `300` 或 `600` 秒。
- 失败重试等待：默认 `30,90`，表示失败后等 30 秒、再失败等 90 秒。
- 每项间隔：默认 `8` 秒，Drive 限流时可调大。
- 最大图片大小：默认 `100` MB，防止异常大文件拖慢任务。
- 最小剩余磁盘：默认 `1024` MB，避免写满电脑磁盘。

设置保存在本机 `data\local_settings.json`，保存后下一次下载任务立即生效。环境变量仍然优先于页面配置。
