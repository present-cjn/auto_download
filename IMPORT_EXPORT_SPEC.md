...
| 4 | Design Link | 当前必填 | `Asset.source_url` | Google Drive 设计图链接，用于自动下载图片。 |
| 5 | Mockup Link | 选填 | `Asset.source_url` | Google Drive mockup 图片链接；有值时也会下载。 |
...
- 如果 `Design Link` 和 `Mockup Link` 指向同一个文件夹，系统仍会创建两条下载任务；图片同名时用 `(1)` 后缀保留重复文件，避免漏下载。
- 同一批次内相同 Google Drive folder 可以复用缓存，避免重复请求 Google Drive。
